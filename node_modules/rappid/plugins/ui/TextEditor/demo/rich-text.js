...
function createTextBox(x, y, text, annotations, boxAttrs) {
    ...
    var box = new joint.shapes.standard.Rectangle({
        position: { x: x, y: y },
        attrs: {
            label: _.extend({
                text: text,
...
createTextBox(
    ...
    { fill: '#4B4A67', rx: 30, ry: 30, stroke: '#FE854F', strokeWidth: 5 }
).rotate(-45);
...
graph.on('change', function(cell) {
    if (cell.hasChanged('attrs')) autosize(cell);
});
...
function autosize(element) {
    ...
    var bbox = view.getNodeBoundingRect(text).inflate(padding);
...
paper.on('cell:pointerdblclick', function(cellView, evt) {
    ...
    // `editor` can be undefined. This can happen if the target of the double-click
    // was not a text element. If you want to be fancy (and suppress the error message printed to the console),
...
paper.on('cell:pointerclick', function(view) {
    ...
    var halo = new joint.ui.Halo({
        ...
        boxContent: function(cellView) {
            ...
            return tmpl({
                ...
                chars: cellView.model.prop('attrs/label/text').length
            });
    ...
    boxColor.selectByValue(view.model.attr('body/fill'));
});
...
boxColor.on('option:select', function(option, index, opt) {
    if (opt && opt.ui) {
        if (selectedCellView) {
            selectedCellView.model.attr('body/fill', option.content);
        }
    }
});
