...
// check whether the link can be connected to the certain cell or pinned to the paper
function connectivity(err, command, next) {
    ...
    if (sourceId && targetId) {
        ...
        if (cellRules && (cellRules.allow || cellRules.deny)) {
            ...
            var isInRuleArray = _.includes(cellRules[cellRules.allow ? 'allow' : 'deny'], targetType);
...
function multiplicity(err, command, next) {
    ...
    // return error if any other link has same source and target and multiLinks are not allowed in settings
    var numberLinks = 0;
