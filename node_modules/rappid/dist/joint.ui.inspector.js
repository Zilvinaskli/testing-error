...
(function (exports, $, core_mjs, SelectBox_mjs, SelectButtonGroup_mjs, ColorPalette_mjs) {
   'use strict';
   ...
   var Inspector = core_mjs.mvc.View.extend({
       ...
       options: {
           ...
           renderLabel: undefined, // function(options, path, value, inspector) { return html }
       ...
       renderFieldContent: function(options, path, value) {
           ...
           switch (options.type) {
               ...
               case 'select-box':
                   ...
                   var selectBoxOptions = core_mjs.util.assign({
                       ...
                       selectBoxOptionsClass: [
                           core_mjs.util.addClassNamePrefix('inspector-select-box-options'),
                           options.selectBoxOptionsClass
                       ].filter(function(item) {
                           return !!item;
                       }).join(' ')
                   });
                   widget = new SelectBox_mjs.SelectBox(selectBoxOptions);
                   widget.$el.attr({
                       'data-attribute': path,
                       'data-type': options.type,
               ...
               case 'color-palette':
                   ...
                   var colorPalletteOptions = core_mjs.util.assign({
                       theme: this.options.theme
                   }, core_mjs.util.omit(options, 'type', 'group', 'index', 'options'), {
                       options: options.items,
                       selected: selectedIndex
                   });
                   widget = new ColorPalette_mjs.ColorPalette(colorPalletteOptions);
                   widget.$el.attr({
                       'data-attribute': path,
                       'data-type': options.type
                   });
               ...
               case 'select-button-group':
                   ...
                   widget = new SelectButtonGroup_mjs.SelectButtonGroup(selectButtonGroupOptions);
                   widget.$el.attr({
                       'data-attribute': path,
                       'data-type': options.type,
                       'data-overwrite': options.overwrite
       ...
       renderOwnFieldContent: function(opt) {
           ...
           switch (opt.type) {
               ...
               case 'content-editable':
                   ...
                   var editableContent = core_mjs.util.isString(opt.value)
                       ? opt.value.replace(/\n/g, '<br>').replace(/ /g, '&nbsp;')
                       : '';
       ...
       renderTemplate: function($el, options, path, opt) {
           ...
           if (options.type === 'list') {
               ...
               core_mjs.util.toArray(value).forEach(function(itemValue, idx) {
               ...
               var attributesArray = Object.keys(flatAttributes).map(function(path) {
       ...
       updateDependants: function(path) {
           ...
           core_mjs.util.toArray(this._when[path]).forEach(function(dependant) {
       ...
       unsetProperty: function(path, opt) {
           ...
           if (path == 'attrs') {
               ...
               var oldAttrValue = core_mjs.util.merge({}, cell.get(attribute));
               var newAttrValue = core_mjs.util.unsetByPath(oldAttrValue, nestedAttrPath, '/');
       ...
       updateCell: function($attr, attrPath, opt) {
           ...
           core_mjs.util.forIn(valuesByPath, function(val, path) {
               this.setProperty(path, val.value, core_mjs.util.assign({ overwrite: val.overwrite }, opt));
           ...
           pathsToRemove.forEach(function(path) {
               ...
               this.removeProperty(path, indexes, core_mjs.util.assign({ rewrite: true }, opt));
           }.bind(this));
       ...
       addListItem: function(evt) {
           ...
           // Focus the first input field within the new item.
           $addedItem.find('input:first').focus();
       ...
       isExpressionValid: function(expr) {
           expr = core_mjs.util.omit(expr, 'otherwise', 'dependencies');
       ...
       extractExpressionPaths: function(expr) {
           ...
           expr = core_mjs.util.omit(expr, 'otherwise', 'dependencies');
           return core_mjs.util.uniq(this._extractVariables(expr).concat(dependencies));
       },
...
}(this.joint.ui = this.joint.ui || {}, $, joint, joint.ui, joint.ui, joint.ui));
