...
(function (exports, core_mjs) {
    ...
    var Record = Element.define('standard.Record', {
        ...
        buildMarkup: function(opt) {
            ...
            var padding = core_mjs.util.normalizeSides(attributes.padding);
            var minGroupWidth = 0;
            ...
            var flags = core_mjs.util.assign({ record: this.id }, opt);
        ...
        autoresize: function(opt) {
            ...
            this.resize(Math.max(width, minSize.width), minSize.height, opt);
        },
        ...
        isLinkInvalid: function(link) {
            ...
            if (source.id === id && source.hasOwnProperty('port') && !items[source.port]) { return true; }
            var target = link.target();
            if (target.id === id && target.hasOwnProperty('port') && !items[target.port]) { return true; }
            return false;
        },
        ...
        getIconMarkup: function(item, x, y) {
            var attributes = this.attributes;
            var itemOffset = attributes.itemOffset;
        ...
        removeItem: function(itemId, opt) {
            ...
            if (items.length > 1) {
                ...
                this.prop(parentPathArray, items, core_mjs.util.assign({ rewrite: true }, opt));
        ...
        addItemAtIndex: function(itemId, index, item, opt) {
            ...
            return this.prop(itemsPathArray, newItems, core_mjs.util.assign({ rewrite: true }, opt));
        },
            ...
            groupWidth: {
                set: function(_, refBBox, node) {
                    ...
                    if (metrics.overflow) {
                        ...
                        if (groupIndex === groupsCount - 1) { width += padding.right; }
                    }
                    return { width: width }
                }
            },
    ...
    var RecordView = core_mjs.dia.ElementView.extend({
        ...
        getLinkEnd: function(magnet) {
            return {
                id: this.model.id,
                port: this.findAttribute('item-id', magnet)
            };
        },
        ...
        getMagnetFromLinkEnd: function(end) {
            ...
            if (!itemId) { ElementViewPrototype.getMagnetFromLinkEnd.apply(this, arguments); }
