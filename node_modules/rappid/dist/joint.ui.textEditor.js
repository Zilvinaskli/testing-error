...
(function (exports, $, Backbone, core_mjs) {
    ...
    var TextEditor = core_mjs.mvc.View.extend({
        ...
        init: function() {
            ...
            core_mjs.util.bindAll(this, 'onMousedown', 'onMousemove', 'onMouseup', 'onDoubleClick', 'onTripleClick', 'onKeydown', 'onAfterPaste', 'onAfterKeydown');
        ...
        render: function(root) {
            ...
            var bbox = core_mjs.V(this.options.text).bbox();
            ...
            core_mjs.V(this.options.text).attr('cursor', 'text');
        ...
        hasSelection: function() {
            ...
            return core_mjs.util.isNumber(this.selection.start) &&
                    core_mjs.util.isNumber(this.selection.end) &&
        ...
        getCombinedAnnotationAttrsAtIndex: function(selectionStart, annotations) {
            ...
            core_mjs.util.toArray(annotations).forEach(function(annotation) {
                if (annotation.start === undefined && annotation.end === undefined) {
                    ...
                    core_mjs.V.mergeAttrs(attrs, annotation.attrs);
        ...
        getSelectionAttrs: function(range, annotations) {
            ...
            if (start === end && start === 0) {
                ...
                for (var i = start; i < end; i++) {
                    ...
                    if (commonAttrs && !core_mjs.util.isEqual(commonAttrs, attrs)) {
                        // Attributes differ. Remove those that differ from commonAttrs.
                        commonAttrs = core_mjs.util.flattenObject(core_mjs.V.mergeAttrs({}, commonAttrs));
                        attrs = core_mjs.util.flattenObject(core_mjs.V.mergeAttrs({}, attrs));
                        var result = {};
                        core_mjs.util.forIn(attrs, function(value, key) {
                            if (commonAttrs[key] === attrs[key]) {
                                core_mjs.util.setByPath(result, key, value);
                            }
                        });
                        commonAttrs = result;
                    } else {
        ...
        select: function(startCharNum, endCharNum) {
            ...
            if (!core_mjs.util.isNumber(this.selection.end)) {
                this.selection.end = this.selection.start;
            }

            if (core_mjs.util.isNumber(this.selection.start) && core_mjs.util.isNumber(this.selection.end)) {
        ...
        annotateURL: function(annotations, selectionStart, selectionEnd) {
            ...
            var urlAnnotation = core_mjs.util.assign({ url: url }, this.options.urlAnnotation);
            ...
            if (!core_mjs.util.isEqual(urlAnnotation, annotations[annotations.length - 1])) {
        ...
        getCharNumFromEvent: function(evt) {
            ...
            var localPoint = core_mjs.V(elText).toLocalPoint(clientX, clientY);
            ...
            if (svgCharNum < 0) {
                ...
                if (nearestCharNum > -1) {
                    return this.svgToRealCharNum(nearestCharNum);
                } else {
                    var right = (localPoint.x > 0);
                    ...
                    if (nearestCharNum > -1) {
                        return this.svgToRealCharNum(nearestCharNum) + (right ? 1 : 0);
                    }
                }
                return (localPoint.x < 0 && localPoint.y < (localTextBBox.y + localTextBBox.height)) ? 0 : this.getNumberOfChars();
            }
        ...
        getFontSize: function() {
            ...
            return parseInt(core_mjs.V(this.options.text).attr('font-size'), 10);
        },

        getTextAnchor: function() {

            return core_mjs.V(this.options.text).attr('text-anchor') || '';
        },
        ...
        onRemove: function() {
            ...
            // TODO: Optional?
            core_mjs.V(this.options.text).attr('cursor', '');
        }
        ...
        edit: function(el, opt) {
            ...
            this.options = core_mjs.util.assign({}, opt, { update: update });
            ...
            this.ed = new TextEditor(core_mjs.util.assign({ text: textElement }, opt));
                    ...
                    if (opt.cellView) {
                        ...
                        if (opt.textProperty) {
                            ...
                            if (opt.cellView.model.isLink() && opt.textProperty.substr(0, 'labels'.length) === 'labels') {
                                originalLabelIndex = core_mjs.V($(core_mjs.V(textElement).node).closest('.label')[0]).index();
                            }
                        ...
                        if (originalLabelIndex !== null) {
                            ...
                            var labelElements = core_mjs.V(opt.cellView.el).find('.label');
                        ...
                        core_mjs.V(textElement).text(newText, { annotations: annotations });
                    }
                }, this);
            }
        ...
        applyAnnotations: function(annotations) {
            ...
            if (ed && opt.update) {
                ...
                if (opt.cellView && opt.annotationsProperty) {
                    ...
                    // all the backbone `changed` mechanism works. This is because
                    // the text editor modifies the `annotations` array in-place.
                    opt.cellView.model.prop(opt.annotationsProperty, this.deepCloneAnnotations(annotations), { rewrite: true, textEditor: ed.cid });
                    ed.setAnnotations(annotations);

                } else {

                    core_mjs.V(ed.options.text).text(ed.getTextContent(), { annotations: annotations });
                }
