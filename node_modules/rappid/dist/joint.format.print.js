...
(function ($, core_mjs) {
    ...
    */
    ...
    core_mjs.dia.Paper.prototype.print = function(opt) {

        var options = resolveOptions(opt);
        var pages = preparePages.call(this, options);
        var printAction = function(pages) {
            sendToPrinter.call(this, pages, opt);
        };
        ...
        options.ready(pages, printAction.bind(this), args);
    };
     ...
     */
    core_mjs.dia.Paper.prototype.preparePrintArea = function(areaToPrintBBox, opt) {

        this.trigger('beforeprint', opt);

        var $printArea = $('<div/>').addClass('printarea');
        var $printPaper = $('<div/>').addClass(core_mjs.util.addClassNamePrefix('print-paper')).css('position', 'relative');
        ...
        var printSVGVel = core_mjs.V(this.svg).clone();
        var printLayersVel = printSVGVel.findOne(("." + (core_mjs.util.addClassNamePrefix('layers'))));
        $printPaper.append(printSVGVel.node);

        var sheetSizePx = getSheetSizePx(opt);
        var paperBBox = this.getArea();
        ...
        var tx = ref$1.tx;
        var ty = ref$1.ty;
        var printMatrix = core_mjs.V.createSVGMatrix().translate(tx / sx, ty / sy);

        var ref$2 = getViewBox(paperBBox, areaToPrintBBox, sheetSizePx);
        var scaleToFit = ref$2.scaleToFit;
        var bBox = ref$2.bBox;
        ...
        printLayersVel.attr('transform', core_mjs.V.matrixToTransformString(printMatrix));
    ...
    var sendToPrinter = function(pages, opt) {
    ...
    var preparePages = function(options) {
    ...
    var getSheetSizePx = function(opt) {
        ...
        var margin = core_mjs.util.normalizeSides(opt.margin);
        var sheet = opt.sheet;
    ...
    var resolveOptions = function(opt) {
        ...
        var options = core_mjs.util.defaultsDeep({}, opt, {
            area: null,
            poster: false,
            sheet: {
                width: 210,
    ...
    var splitArea = function(area, chunkSize) {
        ...
        while (rowsHeight < area.height) {
            while (colsWidth < area.width) {
                chunk = core_mjs.g.Rect(area.x + colsWidth, area.y + rowsHeight, w, h);
                res.push(chunk);
                colsWidth += w;
